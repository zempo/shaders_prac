...
float noise1d_mod(vec2 P, float v){
  return sin(P.y + atan(cos(v) / cos(P.x - 11.)) * 303.1415) * 0.5;
}
...
// to subdivide uv space
  ...
  vec2 uv_y = vec2(cos(sin(uv.y*1000.)+cos(uv.x)),uv.y); 
  // float smoothMod(float x, float y, float e){

  uv_y = modPolar(uv_y,GOLDEN_RATIO);
  vec2 uv_yb = modPolar(uv_y,200.);
  // vec2 modPolar(vec2 p, float repetitions) {

  // float p1 = pow(cos(uv_y.x + uv_y.y),uv.y);
  // float p1 = atan(pow(cos(uv_y.x + uv_y.y),uv.y)/pow(sin(uv_y.y * uv_y.x),uv.x));
  float p1 = pow(cos(uv_y.x + uv_y.y),uv.y);
  float p1b = pow(sin(uv_y.x + uv_y.y),uv.x);
  float p2 = atan(pow(cos(uv_y.x + uv_y.y),uv.y)/pow(sin(uv_y.y * uv_y.x),uv.x));
  float p3 = noise1d_mod(uv_y,p1);
  // *scan lines
  // float p2 = atan(pow(cos(uv_y.x + uv_y.y),uv.y)/pow(sin(uv_y.y * uv_y.x),sin(uv.x*3.-rate)+uv.x));
