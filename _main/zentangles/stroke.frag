...
// to subdivide uv space
  ...
  float p1 = cnoise(uv * 2.0 + vec2(rateh, rate) + s1 + s2);
  float p2 = cnoise(uv * 4.0 + vec2(rateq, rateh) - s1 - s2);

  vec3 c1 = vec3(p1 / s2, p1 / s1, uv.y / uv.x);
  vec3 c2 = vec3(p2 / s2, p2 * s1, .15 * uv.x / uv.y);
