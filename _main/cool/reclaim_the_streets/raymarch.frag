...
vec2 rotate_three(vec2 uv, float rotation, vec2 mid) {
    return vec2(
...
// simplified corridor / lamp function
vec2 fb_simplified(vec3 p, float i, float s) {
    vec2 t = vec2(1e5, 0.0); // start with large distance, no material ID

    // --- 1. main corridor (tube in XZ plane) ---
    float corridor = abs(length(p.xz) - 2.0) - 0.2;
    t = vec2(corridor / s, 1.0); // material 1

    // --- 2. lampposts (thin vertical boxes) ---
    vec3 pp = p;
    pp.z = mod(pp.z, 3.0) - 1.5;  // repeat every 3 units along z
    float post = bo_sdf(pp, vec3(0.1, 2.0, 0.1));
    if (post < t.x) t = vec2(post / s, 2.0); // material 2

    // --- 3. glowing lamp sphere ---
    pp.y -= 2.0; // move down to lamp height
    float lamp = length(pp) - 0.1;
    if (lamp < t.x) t = vec2(lamp / s, 3.0); // material 3
...
// ?? basic scene definition (returns SDF distance)
// float mapScene(vec3 p){
//     // one centered box
//     float d1 = bo_sdf(p, vec3(0.25 + cos(u_time)*.2, 0.35, 0.25));
    
//     // another, shifted upward
//     float d2 = bo_sdf(p - vec3(.0, 0.75, sin(u_time)), vec3(0.15, 0.15, 0.15));
...
//         vec3 pos = ro + rd * t;    // current sample point
//         float dist = mapScene(pos); // scene SDF
//         if(dist < 0.001){          // hit threshold
//             return t;              // return distance
//         }
//         t += dist;                 // march forward
...
// to subdivide uv space
  ...
  vec2 uv_r = rotate_three(uv, PI * 0.25 * rateq, vec2(0.));
  ...
  // *****************************************************
  // *****************************************************
  vec2 uv2 = fract(uv_r * 3.)-.5;

    // ray setup
    vec3 ro = vec3(0.0, 0.0, 3.0);   // ray origin (camera)
    vec3 rd = normalize(vec3(uv, -1.5)); // ray dir toward scene
    ...
    vec3 c2 = vec3(0.0);
    if(t > 0.0){
      // if we hit something, c2or based on distance
      c2 = vec3(1.) * exp(-0.05 * t);
    } else {
      // background
      c2 = vec3(0.1, 0.1, 0.15);
    }  

  // ********** 
      // vec3 ro = vec3(0.0, 0.0, 3.0);   // ray origin (camera)
    // vec3 rd = normalize(vec3(uv2, -1.5)); // ray dir toward scene
  // vec2 pc1 = fb_simplified(vec3(.1,.51,.1),.1,.3); 
