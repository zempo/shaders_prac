...
// to subdivide uv space
  ...
  // uv = fract(uv * .5);
  vec2 ud1 = vec2(uv) * .025;
  vec2 ud2 = vec2(uv) * .05 + sin(uv.x * rateq * .01) + rateq; 

  float p1 = cnoise(vec2(cos(uv.x * rate)+sin(uv.x * rateq),cos(uv.y * rateq)+sin(uv.y)));
  float p2 = cnoise(vec2(cos(ud1.x * rate)+sin(ud1.x * rateq),cos(ud1.y * rateq)+sin(ud1.y)));
  vec3 cp1 = pal(
	p1 * p2,
	vec3(1.00, 1.00, 1.00),
	vec3(1.00, 1.00, 1.00),
	vec3(2.00, 2.00, 2.00),
	vec3(0.00, 1.00 + cos(rateq), 0.00)
  ) * pal(	p2 + cos(p1) - ud2.x,
	vec3(1.00, 1.00, 1.00),
	vec3(1.00, 1.00, 1.00),
	vec3(2.00, 2.00, 2.00),
