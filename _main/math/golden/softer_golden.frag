...
const float GOLDEN_ANGLE = 2.39996322973; // 2π/φ ≈ 137.5° in radians
...
void main() {
    ...
    // Apply golden ratio spiral distortion
    float spiralRadius = pow(GOLDEN_RATIO, angle / GOLDEN_ANGLE) * 0.5;
    float spiralDistortion = radius / spiralRadius;
    
    // Add animation with time
    float animatedAngle = angle + u_time * 0.5;
    float animatedRadius = radius * (1.0 + 0.1 * sin(u_time));
    
    // Create spiral pattern
    float spiral = smoothstep(0.95, 1.05, fract(log(animatedRadius) / log(GOLDEN_RATIO) - animatedAngle / GOLDEN_ANGLE));
    
    // Color based on spiral pattern
    vec3 color = mix(
        vec3(0.9, 0.7, 0.2), // Gold color
