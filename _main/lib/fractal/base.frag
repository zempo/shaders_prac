...
// Max iterations (simulates recursion depth)
#define MAX_ITER 100
int mandelbrot(vec2 c) {
    vec2 z = vec2(0.0);
    float t = u_time * 0.001;
    for (int i = 0; i < MAX_ITER; i++) {
        // "Recursive" step: z = z² + c
        z = vec2(z.x * z.x - z.y * z.y, 2.0 * z.x * z.y) + c;
        // Escape condition (|z| > 2.0)
...
vec3 getMandelColor(int iter) {
    if (iter == MAX_ITER) return vec3(0.0); // Black for stable points
    
    float t = (float(iter) / float(MAX_ITER));
    return vec3(t, 0.5 * t, 1.0 - t); // Gradient
}
