...
float getFieldAngle(vec2 uv) {
  float wave = sin(uv.x * 4.) + cos(uv.y * 3.);
...
void main(){
  ...
  vec3 cp1 = pal(
	...
	vec3(1.00, 1.00, 1.00),
	vec3(1.00, 1.00, 1.00),
	vec3(min(cos(rateh * .5) * 3.,1.), min(sin(rateq * .5) * 3.,1.), 1.00),
	vec3(0.15, 1.00, 0.00)
);

  vec3 c1 = mix(vec3(grid / uv.x), vec3(dist, .9,uv_g.x), smoothstep(PI / 2., .2, dist)); 

  c1 = mix(c1, vec3(0.3), grid); // apply grid color

  // c1 = mix(c1, vec3(uv_g,.1) * .001, smoothstep(2., .0, distance(uv, cell_origin)));
