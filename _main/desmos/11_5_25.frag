...
// to subdivide uv space
  ...
  float a = 30.+sin(rateq)*5.;
  float c = sin(a / 8.+rateq)*cnoise(uv*.1+rateq);
  float denom = x;
  if(abs(x) < .001) denom = .001;

  float eqn = pow(y, 2) - (pow(a, 2) * cos(y+(x*c)) * sin(x)) / denom;
  // float line(float x, float y, float line_width, float edge_width){
 ...
 vec3 c1 = vec3(eqn,.001/exp(eqn),1.-sin(x*cnoise(uv*.1+rateq/eqn*.25)));
