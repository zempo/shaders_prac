...
// to subdivide uv space
  ...
  x += cnoise(uv*7.1+rateh);
  y += cnoise(uv*5.1+rateq);

  float r2 = pow(x, 2) + pow(y, 2);
  float epsilon = 0.0001;

  float left = pow(abs(x), x*cos((x+cnoise(uv+rateq)+y)/(r2+epsilon)));
  float right = 1227. * cos((x+pow(y, 2))/(r2+epsilon)) * cnoise(uv+rateq);

  bool condition = left < right;

  vec3 c1;
  if(condition){
    float hue = atan(y, x) / TAU + rateh;
    float dist = length(uv) * 0.1;
    c1 = 0.5 + 0.5 * cos(rate + hue * TAU + vec3(0, 2.-sin(rate), 4.-sin(rate)));
    c1 *= 1. - smoothstep(0.0, 0.3, abs(left - right) / 1000.0) + (.25*abs(sin(rateq)));    
  } else {
    float hue = atan(y, x) / TAU;
    float dist = length(uv) * 0.1;
    vec3 c0 = 0.25 + 0.5 * sin(rate + hue * TAU + vec3(0, 1, 2));
    // c1 *= smoothstep(0.05, 0.3, abs(left - right) / 1000.0) + (.25*(sin(rateq))) *.1; 
    c1 = vec3(.1, .2, .21) * pal(
	  cnoise(uv*.2+rateq)+dist+hue,
	vec3(0.79, 0.79, 0.79),
	vec3(0.50, 0.50, 0.50),
	vec3(1.00, 1.00, 1.00),
	vec3(0.00, 0.33, 0.67)
    ...
    if (abs(x) < 0.1) c1 = mix(c1, vec3(0.1373, c1.g, 0.1608)-c1, 0.6);
    if (abs(y) < 0.1) c1 = mix(c1, vec3(c1.r, 0.3804, c1.b)*c1, 0.6);
    
    // Add circular grid
    for (float r = 5.0; r <= 15.0; r += 5.0) {
        if (abs(length(uv) - r) < 0.1) {
            c1 = mix(c1, vec3(0.3), .4);
        }
    }  
