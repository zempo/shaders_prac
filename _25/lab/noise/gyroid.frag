...
// vec3 c1 = vec3(uv.y*4., (rateq * .25), uv.x*2.);
// float p1 = fbm(c1, 0.01);
...
// vec3 cp2 = pal(
// 	cnoise((uv * 1.2 + rateq)) + p1,
// 	vec3(0.09 + sin(rateq), 0.50 + cos(rateq), 0.98 - cos(rateq)),
// 	vec3(0.50, 0.50, 0.50),
// 	vec3(1.00, 1.00, 1.50),
// 	vec3(0.00, 0.33, 0.67)
// );
...
//  ??? PERM 4: rainbow clouds  glslViewer -l _25/lab/noise/gyroid.frag noise.png
// vec3 t1 = texture2D(u_tex1, uv).rgb;
...
// vec3 c1 = vec3(uv.y*3., (rateq * .25) + uv.y*.34, uv.x*2.);
// float p1 = fbm(c1, 0.01);
  
// vec3 cp1 = pal(
// 	p1,
// 	vec3(0.09 + sin(rateq), 0.50 + cos(rateq), 0.98),
// 	vec3(0.50, 0.50, 0.50),
// 	vec3(1.00, 1.00, 1.00),
// 	vec3(0.00, 0.33, 0.67)
// );
